...
const SERVER_ORIGIN = (import.meta.env.VITE_API_URL || 'https://api.cnmvchat.click/api').replace(/\/api\/?$/, '');
...
const Chat = () => {
    ...
    const filteredFriends = friends.filter(friend => friend.user && (friend.user.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) || friend.user.email?.toLowerCase().includes(searchTerm.toLowerCase())));
